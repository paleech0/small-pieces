...
- **问询预览**：CC 向用户提问（AskUserQuestion）时，弹窗展示问题内容和选项列表，确认后由 CC 原生界面收答案
...
开启后所有弹窗不再抢占焦点，而是最小化到任务栏。点击任务栏图标并点击恢复按钮即可恢复窗口进行操作。
...
| 环境 | 是否可用 | 说明 |
|------|----------|------|
| CC 桌面端 (Windows/Mac) | ✅ | 原生支持，前台检测准确 |
| CC CLI (Windows Terminal 等) | ✅ | 同上 |
| CC + Cursor 内置终端 | ✅ | 弹窗正常，前台检测可能误判 |
| CC + VS Code 内置终端 | ✅ | 同 Cursor |
| Codex 桌面端/插件 | ❌ | OpenAI 产品，不同钩子系统，不兼容 |
| 纯 SSH / 无 GUI | ❌ | tkinter 需要桌面环境 |
